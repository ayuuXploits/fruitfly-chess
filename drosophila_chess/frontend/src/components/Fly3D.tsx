...
/* ─── Biomechanical NeuroMechFly PBR Materials ─── */
const chitinBodyMat = new MeshPhysicalMaterial({
  color: '#e8a153', // NeuroMechFly honey-tan thoracic chitin
  roughness: 0.35,
  metalness: 0.12,
  clearcoat: 0.55,
...
const eyeRubyMat = new MeshPhysicalMaterial({
  color: '#dc2626', // vermilion/ruby compound eyes
  emissive: '#991b1b',
...
const pedestalWoodMat = new MeshStandardMaterial({
  color: '#0f1118', // Deep matte dark wood
  roughness: 0.75,
  metalness: 0.05,
})
...
export default function Fly3D({ isThinking, activeMove, onPiecePlaced }: Props) {
  ...
  // Perch Position elevated so tarsal claws rest precisely on platform plate (y = 0.40)
  const PERCH_POS = useMemo(() => new Vector3(0, 1.07, -4.85), [])
  ...
  useEffect(() => {
    ...
    // Keyframes for the flight spline
    const pStart = PERCH_POS.clone()
    const pLiftOff = new Vector3(fx * 0.3, 1.65, (PERCH_POS.z + fz) * 0.5)
    const pHoverFrom = new Vector3(fx, fy + 1.15, fz)
    const pGraspFrom = new Vector3(fx, fy + 0.85, fz) // touch & grab piece
    const pMidAir = new Vector3((fx + tx) * 0.5, 1.95, (fz + tz) * 0.5)
    const pHoverTo = new Vector3(tx, ty + 1.15, tz)
    const pPlaceTo = new Vector3(tx, ty + 0.85, tz) // set down piece
    const pReturnClimb = new Vector3(tx * 0.3, 1.65, (PERCH_POS.z + tz) * 0.5)
  ...
  useFrame((state, delta) => {
    ...
    if (isFlying.current && flightSpline.current) {
      ...
      // Perched idle on pedestal overlooking board
      if (isThinking) {
        // Leaning forward with buzzing wings and glowing eyes
        ...
        if (flyRootRef.current) {
          ...
          flyRootRef.current.rotation.set(0.25, Math.sin(t * 3) * 0.04, 0)
        }
      } else {
        currentPos.current.copy(PERCH_POS)
        if (flyRootRef.current) {
          ...
          flyRootRef.current.rotation.set(0.14, Math.sin(t * 1.5) * 0.02, 0)
        }
      }
    }
  })
  ...
  return (
    <group>
      {/* ── Sleek Dark Observation Platform (No golden lines, no curved rings) ── */}
      <group position={[PERCH_POS.x, 0, PERCH_POS.z]}>
        {/* Tier 1 - Dark ebony plinth base */}
        <mesh position={[0, 0.11, 0]} material={pedestalWoodMat} castShadow receiveShadow>
          <cylinderGeometry args={[0.95, 1.05, 0.22, 32]} />
        </mesh>
        {/* Tier 2 - Upper dark wood tier */}
        <mesh position={[0, 0.28, 0]} material={pedestalWoodMat} castShadow>
          <cylinderGeometry args={[0.88, 0.95, 0.16, 32]} />
        </mesh>
        {/* Top platform disc - smooth dark slate/graphite plate (top surface = 0.40) */}
        <mesh position={[0, 0.39, 0]} material={platformTopMat} receiveShadow>
          <cylinderGeometry args={[0.90, 0.90, 0.02, 32]} />
        </mesh>
      </group>
